"""
Сервис фиксации акцепта Пользовательского соглашения и Публичной оферты.

Логика:
  - "Требуется принять" считается по совпадению версии в CURRENT_VERSIONS
    с последней принятой пользователем версией данного документа.
...
class LegalConsentService:
    ...
    async def get_status(user_id: int) -> dict:
        async with async_session() as session:
            ...
            result["all_accepted"] = all(result[d]["accepted"] for d in ALL_DOCUMENT_TYPES)
            return result
    ...
    async def record_consent(
        ...
        return await LegalConsentService.get_status(user_id)
