...
class AdminReportRouter:
    ...
    def add_get_actors_route(self,):
        ...
        async def get_actors_route(
            ...
            """
            - Если дернешь эту ручку, когда каст лист еще не заполнялся актерами, то тебе придут все возможные актеры
            по соответствующим фильтрам

            - Если дернешь эту ручку, когда каст лист уже заполнен актерами, например, при редактировании каст листа, то тебе
            будут приходить только те актеры, которые не были добавлены в него ранее. Так ты можешь отправлять запрос
            на актуализацию листинга после каждого действия внутри каст листа
            """
