 ...
 * обработки персональных данных, Согласие на обработку персональных данных
 * и Согласие на трансграничную передачу персональных данных на самом первом
 * экране — раньше выбора роли и входа (см. «Комплект по ролям» в инструкции
 * по внедрению документов: этот базовый набор общий для Актёра, Агента и
 * Администратора/PRO — трансграничная передача данных в Railway/Resend/
 * Telegram происходит фактически сразу при регистрации). Публичная оферта в
 * этот набор не входит — она нужна только Администратору/PRO и проверяется
 * отдельно, ролевым гейтом после входа (см. legal-consent-gate). Аккаунта в
 * момент этого экрана ещё нет, поэтому зафиксировать акцепт на сервере
 * нельзя: сохраняем отметку локально вместе с редакциями документов. После
 * входа `legal-consent-gate` отправляет её на сервер, чтобы не спрашивать
 * согласие второй раз.
 */
export type LegalDocType = 'user_agreement' | 'privacy_policy' | 'data_processing_consent' | 'cross_border_consent'
