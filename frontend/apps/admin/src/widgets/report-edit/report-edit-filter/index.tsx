...
import { ReportEditFilterActions } from './report-edit-filter-actions'

export const ReportEditFilter = ({ report }: WithReportID) => {
	const { filter, checked, list } = useReportEditStore()
			...
			<Flex justifyContent={'space-between'}>
				<Flex alignItems={'center'} gap={14}>
					<Checkbox
						...
						checked={list.every(item => checked.includes(item.id))}
						onChange={handleAll}
					/>
					...
					>
						...
						<FormRow label={'Актеры'}>
							<ReportViaCastingSelect
								...
								onChange={e => handleFilter(e, 'via_casting')}
							/>
						</FormRow>
				...
				>
					<ReportEditFilterActions report={report} />
				</Dropdown>
			</Flex>
		</Card>
	)
