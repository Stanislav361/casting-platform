...
import { Dictionary, ModelListField } from '@prostoprobuy/types'
...
export const createModelListApi = <T, U extends Dictionary<any>>(
	...
	const setActions = createEvent<number[]>()
	...
	$store.on(setActions, (state, actions) => ({ ...state, actions }))
	...
	$store.on(reset, state => ({
		...
		filter: {
			...
			page_number: state.filter?.page_number,
		},
	}))
